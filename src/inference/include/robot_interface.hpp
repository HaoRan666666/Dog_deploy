...
class RobotInterface {
   public:
    ...
    //
    // 读回当前编码器位置, 以 v=0、默认 kp/kd 回发 MIT 命令:
    //   - 位置项 kp×(p_des-p_actual) ≈ 0, 不产生主动力矩
    //   - 速度项 kd×(0-v_actual) 提供阻尼, 抑制扰动
    //   - 轮电机 (kp=0) 退化为纯阻尼, 停止转动
    //
    // 作用: 喂电机 CAN 通信看门狗, 防止电机使能后因长时间收不到 MIT 帧而
    //       自动失力 (无力); 同时保持当前姿态不塌。
    // =========================================================================
    void hold_position();
    ...
    //        此时 control 线程应跳过 hold_position(), 避免保持命令与
    //        插值命令 (40% KP) 相互干扰。
    // false: 无阻塞操作, control 线程可正常下发 hold 命令。
    // =========================================================================
