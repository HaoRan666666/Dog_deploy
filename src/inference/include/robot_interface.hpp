...
class RobotInterface {
   public:
    ...
    //
    // 以 kp=kd=tau=0 回发 MIT 命令: 电机保持使能状态 (不断帧, 不触发 CAN
    // 通信看门狗超时失力), 但不施加任何保持力矩, 关节可自由转动 (趴着软)。
    //
    // 作用: 电机使能后、尚未起身站立前, 持续喂狗防止超时失力。
    // =========================================================================
    void apply_zero_torque();
    ...
    // =========================================================================
    // is_operating_ — 电机阻塞操作进行中标志 (stand_up / reset_joints)
    //
    // true:  main 线程正在执行 stand_up() / reset_joints() 插值动作。
    //        此时 control 线程应跳过下发 (保持站立/零力矩), 避免与
    //        插值命令 (40% KP) 相互干扰。
    // false: 无阻塞操作, control 线程可正常下发命令。
    // =========================================================================
    std::atomic<bool> is_operating_{false};

    // =========================================================================
    // is_stood_up_ — 是否已起身站立 (电机使能后是否完成过 stand_up)
    //
    // true:  已完成起身, 机器人处于默认站立姿态 (stand_up/reset_joints 后)
    // false: 刚使能电机、尚未起身 (init_motors 后), 机器人趴着
    //
    // 作用: control 线程区分"趴着零力矩喂狗"与"站立满 KP 保持默认姿态"
    // =========================================================================
    std::atomic<bool> is_stood_up_{false};
