...
// ============================================================================
namespace {
struct OperatingScope {
    std::atomic<bool>& flag_;
    explicit OperatingScope(std::atomic<bool>& f) : flag_(f) { flag_.store(true); }
    ~OperatingScope() { flag_.store(false); }
};
}
    ...
    for (size_t i = 0; i < motors_cfg_->motor_interface_.size(); ++i) {
        for (size_t j = 0; j < motors_cfg_->motor_num_[i]; ++j) {
            motors_[count] = MotorDriver::create_motor(
                ...
                motors_cfg_->motor_type_[count],         // 电机类型 (每电机一个, 如 "LRO_CAN"/"LINGZU")
